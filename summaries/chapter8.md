# Chaptre 8: Reading And Writing Files.

## Summaries What I Learn In This Chapter
- Files and File Paths
- The os.path Module
- The file raeding/writing process
- The open() Function
- Saving variable with the shelve module
- Saving variable with the pprint.pformat() Function
- Generating Random Quiz files: [Project](../Code/Projects/chapter8_project1.py)
- MuticlipBoard: [Project](../Code/Projects/chapter8_project2.py)

## [Projects](../images/ch8_Practice_Projects.png)
- [Project 1](../Code/Projects/chapter8_practice_project1.py)
- [Project 2](../Code/Projects/chapter8_practice_project2.py)
- [Project 3](../Code/Projects/chapter8_practice_project3.py)
  
